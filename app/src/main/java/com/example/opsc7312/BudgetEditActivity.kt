...
class BudgetEditActivity : ComponentActivity() {
    ...
    private fun handleEditBudgetClick(userId: String?) {
        ...
        private fun updateBudget(
            ...
            val updateBudgetAmountCall = RetrofitClient.apiService.editBudgetAmount(
                userId,
                accountName,
                category,
                amountBudgeted
            ...
            updateBudgetAmountCall.enqueue(object : Callback<UpdateBudgetAmountResponse> {
                override fun onResponse(
                    call: Call<UpdateBudgetAmountResponse>,
                    response: Response<UpdateBudgetAmountResponse>
                ) {
                    if (response.isSuccessful && response.body() != null) {
        ...
        private fun updateSpentAmount(
            ...
            val updateSpentAmountCall = RetrofitClient.apiService.editSpentAmount(
                userId,
                accountName,
                category,
                amountSpent
            ...
            updateSpentAmountCall.enqueue(object : Callback<UpdateSpentAmountResponse> {
                override fun onResponse(
                    call: Call<UpdateSpentAmountResponse>,
                    response: Response<UpdateSpentAmountResponse>
                ) {
                    if (response.isSuccessful && response.body() != null) {
                        showToast(response.body()!!.message)
                    } else {
        ...
        private fun saveBudgetActionToRoom(
            userId: String,
            accountName: String,
            category: String,
            amountBudgeted: Double,
